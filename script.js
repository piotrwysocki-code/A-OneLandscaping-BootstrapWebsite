  ...
  if (connectObj) {
    ...
    $.ajax({
      ...
      if (results.Success) {
        ...
        $("#success-message")
          .html(
            `Your message has been successfully delivered, a confirmation email has been sent to ${connectObj.email}`
          )
          .fadeIn("slow");
        setTimeout(() => {
...
validateForm = () => {
  $("#name, #email, #phone, #service, #message").removeClass(
    "is-valid is-invalid"
  );
...
function captchaError() {
  $(
    "#captcha-info, #captcha-error, #captcha-expired, #captcha-failed, #success-message"
  ).hide();
